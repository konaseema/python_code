...
def localize(colors,measurements,motions,sensor_right,p_move):
    ...
    #print 'p has', len(p), 'rows'
    #print 'p has', len(p[0]), 'columns'
    # >>> Insert your code here <<<
    # analogy to q in 1d example

    def move(p, motion, p_move):
        q = [[1 for row in range(len(colors[0]))] for col in range(len(colors))]
        #print 'q has', len(colors), 'rows'
        #print 'q has', len(colors[0]), 'columns'
        Y = motion[0]
        X = motion[1]

        for i in range(len(colors)):
            for j in range(len(colors[0])):
                #print 'i',i,'j',j, 'x index',(i-X) % len(colors[0]), 'y index', (j-Y) % len(colors)
                #print p[(j-Y) % len(colors)][(i-X) % len(colors[0])]
